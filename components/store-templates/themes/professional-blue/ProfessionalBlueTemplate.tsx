...
export default function ProfessionalBlueTemplate({
  ...
  contactPhone, contactEmail, contactAddress, facebookUrl, instagramUrl, whatsappUrl, snapchatUrl, tiktokUrl, telegramUrl, linkedinUrl, themeMeta, colors,
}: TemplateProps) {
  ...
  <div className="flex items-center gap-6">
      ...
      <header className="sticky top-0 z-40 bg-white" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.07)" }}>
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 flex items-center gap-1.5 sm:gap-4 h-16">
          <button className="md:hidden p-1 -ml-1" onClick={() => setMobileOpen(!mobileOpen)} style={{ color: "#374151" }}>{mobileOpen ? <CloseIcon size={22} /> : <MenuIcon size={22} />}</button>
          ...
          <div className="flex items-center gap-0.5 sm:gap-1 ms-auto shrink-0">
            <button className="md:hidden p-1 sm:p-2 rounded-full" onClick={() => setSearchOpen(!searchOpen)} style={{ color: "#374151" }}><SearchIcon size={18} /></button>
            <a href={`/store/${slug}/track-order`} className="p-1 sm:p-2 rounded-full hover:bg-gray-100" style={{ color: "#374151" }} aria-label={t("storefront.trackOrder")}>
              <TruckIcon size={18} />
            </a>
            <QuickLoginButton slug={slug} compact />
            <a href={`/store/${slug}/wishlist`} className="relative p-1 sm:p-2 rounded-full hover:bg-gray-100" style={{ color: "#374151" }} aria-label={t("storefront.wishlist")}>
              <HeartIcon size={18} />
            ...
            <a href={`/store/${slug}/cart`} className="relative p-1 sm:p-2 rounded-full hover:bg-gray-100" style={{ color: "#374151" }} aria-label={t("storefront.cart")}><BagIcon size={18} />
