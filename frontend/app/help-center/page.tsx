...
export default function HelpCenterPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError("");
      try {
        const res = await fetch(`${API_BASE}/site/pages/help-center`);
  ...
  return (
    <SiteLayout>
      <div>
        ...
        >
          <h1 className="text-3xl sm:text-4xl font-bold mb-3">{t("page.helpCenter")}</h1>
          <p className="text-[15px] opacity-80">{t("common.help")}</p>
        </section>
        <div className="max-w-4xl mx-auto px-4 py-12">
          <div className="bg-white border border-[var(--border)] rounded-[var(--radius-md)] shadow-[var(--shadow)] p-8 md:p-12">
            {data?.title && (
