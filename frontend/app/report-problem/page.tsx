...
export default function ReportProblemPage() {
  ...
  const PROBLEM_TYPES = [
    { value: "Technical", label: t("reportProblem.technical") },
    { value: "Payment", label: t("reportProblem.payment") },
    { value: "Suggestion", label: t("reportProblem.suggestion") },
    { value: "Other", label: t("reportProblem.other") },
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/site/contact`, {
        ...
        body: JSON.stringify({ ...form, type: "ReportProblem" }),
      });
      const json = await res.json();
      if (!res.ok) {
        throw new Error(json.message || t("reportProblem.submitError"));
      }
      setSuccess(t("reportProblem.success"));
      setForm({ name: "", email: "", phone: "", problemType: "Technical", subject: "", message: "" });
    } catch (err: any) {
      setError(err.message || t("reportProblem.submitError"));
    } finally {
      setLoading(false);
    }
  };
      ...
      <SiteLayout>
        <div className="min-h-[60vh] flex items-center justify-center px-4">
          <div className="max-w-lg w-full p-10 text-center rounded-[var(--radius-md)] shadow-[var(--shadow)] border border-green-200 bg-white">
            ...
            <h2 className="text-xl font-bold text-[var(--ink)] mb-3">{t("reportProblem.thankYou")}</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              <div className="bg-white border border-[var(--border)] rounded-[var(--radius-md)] shadow-[var(--shadow)] p-8 md:p-10">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      ...
                      <div className="field-shell">
                        <input name="name" value={form.name} onChange={handleChange} placeholder={t("reportProblem.namePlaceholder")} required />
                      </div>
                    </div>
                    <div>
                      <label>{t("auth.email")}</label>
                      <div className="field-shell">
                        <input name="email" type="email" value={form.email} onChange={handleChange} placeholder={t("reportProblem.emailPlaceholder")} required />
                      </div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div className="field-shell">
                        <select name="problemType" value={form.problemType} onChange={handleChange} className="w-full bg-transparent" style={{ color: "var(--ink)" }}>
                  ...
                  <div>
                    <label>{t("reportProblem.subject")}</label>
                    <div className="field-shell">
                      <input name="subject" value={form.subject} onChange={handleChange} placeholder={t("reportProblem.subjectPlaceholder")} required />
                    </div>
                  </div>
                  <div>
                    <label>{t("reportProblem.message")}</label>
                    <div className="field-shell">
                      <textarea name="message" value={form.message} onChange={handleChange} placeholder={t("reportProblem.messagePlaceholder")} rows={5} required />
                    </div>
                  </div>
                  <button type="submit" disabled={loading} className="btn btn-primary w-full justify-center">
                    {loading ? t("reportProblem.sending") : t("reportProblem.submit")}
                  </button>
                </form>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="bg-white border border-[var(--border)] rounded-[var(--radius-md)] shadow-[var(--shadow)] p-6">
                <h3 className="text-[15px] font-bold text-[var(--ink)] mb-4">{t("reportProblem.contactInfo")}</h3>
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[12px] text-[var(--sub)]">{t("reportProblem.whatsapp")}</p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[12px] text-[var(--sub)]">{t("reportProblem.workHours")}</p>
                      <p className="text-[13px] font-bold text-[var(--ink)]">{t("reportProblem.workHoursValue")}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border border-[var(--border)] rounded-[var(--radius-md)] shadow-[var(--shadow)] p-6">
                <h3 className="text-[15px] font-bold text-[var(--ink)] mb-4">{t("reportProblem.followUs")}</h3>
                <div className="flex items-center gap-3">
