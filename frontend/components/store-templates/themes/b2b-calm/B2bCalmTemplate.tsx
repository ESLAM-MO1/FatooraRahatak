...
import { BagIcon, HeartIcon, HeartFilledIcon, MenuIcon, CloseIcon, MailIcon, PhoneIcon, MapPinIcon, FacebookIcon, InstagramIcon, WhatsAppIcon, PackageIcon, CheckIcon, LeafIcon } from "@/components/store-templates/icons";
...
export default function B2bCalmTemplate({
  ...
  const { categories, products, productsLoading, selectedCategoryId, setSelectedCategoryId, showAllProducts, setShowAllProducts,
    cartCount, wishlist, toggleWishlist, isSearchActive, searchResults, currencySymbol, isRtl, handleAddToCart, cartMessage, cartMessageType, quoteRequested, handleQuoteRequest } = useStorefront(slug, storeId, currency, true, showHero, themeMeta);
  ...
  const cats = categories.length ? categories : [{ id: null, nameAr: "الكل", image: null }, { id: null, nameAr: "الأكثر مبيعًا", image: null }, { id: null, nameAr: "عروض خاصة", image: null }];
  ...
  return (
    <div dir={isRtl ? "rtl" : "ltr"} className="font-sans" style={{ fontFamily: "'Segoe UI', 'Tajawal', system-ui, sans-serif", background: "#F7FAF9" }}>
      <header className="sticky top-0 z-40" style={{ background: "rgba(255,255,255,0.9)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", borderBottom: "1px solid #E3EDEA" }}>
        ...
        {mobileOpen && <div className="md:hidden border-t" style={{ borderColor: "#E3EDEA", background: "#fff" }}><div className="px-4 py-3 space-y-2 text-[15px] font-semibold" style={{ color: colors.headerColor }}>
      ...
      {showHero && (
        <section className="w-full" style={{ background: `linear-gradient(120deg,${colors.heroFrom} 0%,${colors.heroTo} 100%)`, color: "#fff" }}>
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-20 text-center">
            <span className="inline-flex items-center gap-2 px-5 py-1.5 text-[13px] font-bold rounded-full" style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)" }}><LeafIcon size={14} />{t("storefront.heroWelcomeB2B")}</span>
            <h1 className="mt-6 font-bold leading-tight" style={{ fontSize: "clamp(34px,5vw,52px)" }}>{storeName}</h1>
            <p className="mt-4 text-[15px]" style={{ color: "rgba(255,255,255,0.9)" }}>{t("storefront.heroWelcome")}</p>
            <a href="#products" onClick={(e) => { e.preventDefault(); document.getElementById("products")?.scrollIntoView({ behavior: "smooth" }); }} className="inline-block mt-8 px-10 py-3 font-bold text-sm rounded-full" style={{ background: "#FFFFFF", color: colors.headerColor }}>
      ...
      {!isSearchActive && showHero && (
        <section style={{ padding: "28px 0" }}>
          <div className="max-w-6xl mx-auto px-4 sm:px-6 flex gap-3 overflow-x-auto">
            {cats.map((cat, idx: number) => (
              <button key={idx} onClick={() => { if (cat.id !== null) setSelectedCategoryId(selectedCategoryId === cat.id ? null : cat.id); }}
                className="shrink-0 px-5 py-2 text-[13px] font-semibold rounded-full transition-colors"
                style={{ background: selectedCategoryId === cat.id ? colors.accentColor : "#fff", color: selectedCategoryId === cat.id ? "#fff" : colors.headerColor, border: "1px solid #D8E6E2", boxShadow: selectedCategoryId === cat.id ? "none" : "0 2px 8px rgba(0,0,0,0.04)" }}>
                {cat.nameAr}
              </button>
            ))}
          </div>
      ...
      {showHero && <section id="products" style={{ padding: "32px 0 48px" }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <h2 className="font-bold text-xl" style={{ color: colors.headerColor }}>{t("storefront.featuredProducts")}</h2>
          ...
          {!productsLoading && displayProducts.length > 0 && <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {displayProducts.map(p => (
              <div key={p.id} className="bg-white rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-lg" style={{ border: "1px solid #E3EDEA", boxShadow: "0 4px 16px rgba(0,0,0,0.04)" }}>
                ...
                <div className="p-5">
                  ...
                  <div className="mt-4 flex gap-2">
                    <button onClick={() => handleQuoteRequest(p.id)} className="flex-1 py-2.5 text-[13px] font-bold rounded-xl" style={{ background: quoteRequested === p.id ? "#22C55E" : "transparent", border: `1px solid ${colors.accentColor}`, color: quoteRequested === p.id ? "#fff" : colors.accentColor }}>
                      {quoteRequested === p.id ? <span className="inline-flex items-center gap-1.5"><CheckIcon size={14} />{t("storefront.quoteSentDone")}</span> : t("storefront.requestQuote")}
                    </button>
                    <button onClick={() => handleAddToCart(p.id)} className="flex-1 py-2.5 text-[13px] font-bold rounded-xl" style={{ background: colors.buttonColor, color: "#fff" }}>
        ...
        <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <div className="mt-3 flex gap-3">
              {fbUrl && <a href={fbUrl} target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="flex items-center justify-center w-9 h-9 rounded-full" style={{ background: "rgba(255,255,255,0.15)", color: "#fff" }}><FacebookIcon size={16} /></a>}
              {igUrl && <a href={igUrl} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="flex items-center justify-center w-9 h-9 rounded-full" style={{ background: "rgba(255,255,255,0.15)", color: "#fff" }}><InstagramIcon size={16} /></a>}
              {waUrl && <a href={waUrl} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" className="flex items-center justify-center w-9 h-9 rounded-full" style={{ background: "rgba(255,255,255,0.15)", color: "#fff" }}><WhatsAppIcon size={16} /></a>}
            </div>
          </div>
        </div>
