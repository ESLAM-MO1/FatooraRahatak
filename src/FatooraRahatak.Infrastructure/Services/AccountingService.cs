   ...
   public async Task<List<AccountDto>> GetAccountsTreeAsync(long userId)
{
    ...
    // ⚠️ عكس القيد المحاسبي عند إرجاع طلب — يدعم الإرجاع الكامل والجزئي:
    // - returnRequestId == null: سلوك قديم (توافقًا)، يُعامل كإرجاع كامل دائمًا.
    // - returnRequestId != null: تُحسب نسبة المبلغ المرتجع (من عناصر طلب الإرجاع هذا مقابل
    //   SubTotal الطلب الكلي)، وتُطبَّق نفس النسبة على كل بند في القيد الأصلي (الإيراد، الضريبة،
    //   تكلفة البضاعة، عمولة المنصة، صافي المستحق للتاجر) باستثناء بنود الشحن، التي لا تُعكس أبدًا
    //   في الإرجاع الجزئي لأن العميل استخدم خدمة الشحن بالفعل.
    public async Task ReverseOrderSalesInvoiceAsync(long storeId, long orderId, long? returnRequestId = null)
    {
        var order = await _context.Orders
            .Include(o => o.Items)
            .FirstOrDefaultAsync(o => o.Id == orderId && o.StoreId == storeId);
        ...
        if (returnRequestId.HasValue)
        {
            ...
            if (returnRequest != null && order.SubTotal > 0)
            {
                var returnedItemIds = returnRequest.Items.Select(i => i.OrderItemId).ToHashSet();
                var returnedSubTotal = order.Items
                    .Where(oi => returnedItemIds.Contains(oi.Id))
                    .Sum(oi => oi.LineTotal);

                proportion = Math.Round(returnedSubTotal / order.SubTotal, 6);
                if (proportion > 1m) proportion = 1m;
            }
        }

        var isFullReturn = proportion >= 0.999999m;
        ...
        if (isFullReturn)
        {
            ...
            var reversal = new JournalEntry
            {
                ...
                EntryNumber = entryNumber,
                EntryDate = DateOnly.FromDateTime(DateTime.UtcNow),
                Description = $"قيد عكسي كامل للقيد رقم {original.EntryNumber}",
                Status = JournalEntryStatus.Approved,
            ...
            if (invoice != null)
            {
                ...
                invoice.RefundedAmount = invoice.TotalAmount;
        ...
        else
        {
            ...
            var linesToReverse = original.Lines
                .Where(l => !l.LineDescription.Contains("الشحن"))
                .ToList();

            if (linesToReverse.Count == 0)
                return;
            ...
            var reversal = new JournalEntry
            {
                ...
                EntryNumber = entryNumber,
                EntryDate = DateOnly.FromDateTime(DateTime.UtcNow),
                Description = $"قيد عكسي جزئي للقيد رقم {original.EntryNumber} — إرجاع جزء من طلب {order.OrderNumber}",
                Status = JournalEntryStatus.Approved,
                CreatedByUserId = original.CreatedByUserId,
                ApprovedByUserId = original.CreatedByUserId,
                ReversalOfEntryId = original.Id,
                IsAutoGenerated = true,
                SourceType = JournalSourceType.SalesInvoice,
                Lines = linesToReverse.Select(l => new JournalEntryLine
                {
                    AccountId = l.AccountId,
                    Debit = Math.Round(l.Credit * proportion, 2),
                    Credit = Math.Round(l.Debit * proportion, 2),
                    LineDescription = $"{l.LineDescription} (إرجاع جزئي)"
                }).ToList()
            };
            ...
            if (invoice != null)
            {
                var refundedNow = Math.Round(invoice.TotalAmount * proportion, 2);
                invoice.RefundedAmount = Math.Round(invoice.RefundedAmount + refundedNow, 2);
                invoice.PaymentStatus = invoice.RefundedAmount >= invoice.TotalAmount
